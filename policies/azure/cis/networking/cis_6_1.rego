package argus.azure.cis.cis_6_1
...
metadata := {
	...
	"severity": "CRITICAL",
	"pillar": "Network",
	"title": "Ensure SSH (port 22) is not exposed to the internet",
	"description": "NSG inbound rules must not allow TCP port 22 from '*' or '0.0.0.0/0'. Exposed SSH is the #1 cause of compromise for Azure Linux VMs.",
	"zt_tenet": "Tenet 2",
...
matches_port_22(rule) if {
	range := rule.properties.destinationPortRange
	range == "*"
}
...
violation contains msg if {
	...
	msg := {
		...
		"detail": sprintf("NSG '%v' rule '%v' allows inbound SSH from the internet. This exposes VMs to global brute force.", [nsg.name, rule.name]),
		"evidence": {
			"nsg_id": nsg.id,
			"rule_name": rule.name,
			"protocol": proto,
